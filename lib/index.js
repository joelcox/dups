...
Server.prototype._bootstrap = function() {
  ...
  this.socket.on('listening', this._runInit);
  this.socket.on('message', this._processMessage);
}
...
Server.prototype._runInit = function() {
  ...
  // This points to the EventStream rather than the Server object, gotta
  // bind this in _bootstrap!
...
Server.prototype._processMessage = function(buffer, requestInfo) {
  ...
  req = new this.Response(buffer, requestInfo);

  // Check if there is a handler for this command
  if (typeof this.handlers[req.command] === 'undefined') {
    throw new Exception('No handler for command ' + req.command);
  }
