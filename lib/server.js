 ...
 * @param {string} name Name for the callback method
 * @param {function} fn Callback function
 * @api public
 */
Server.prototype.every = function(interval, name, fn) {
  assert.deepEqual(typeof interval, 'number', 'First argument must be a number');
  assert.deepEqual(typeof name, 'string', 'Second argument must be a string')
...
Server.prototype._startIntervals = function() {
  ...
  Object.keys(handlers).forEach(function(key) {
    currentHandle = handlers[key];
    if (typeof handlers[key].interval === 'undefined') return;
    handlers[key].intervalId = setInterval(currentHandle.fn, currentHandle.interval, req);
  });
